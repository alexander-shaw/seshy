...
// Manages the overall user session and app state.
// Provides a reactive interface for SwiftUI views to handle session management including:
// - User authentication state tracking;
// - App phase management (splash, onboarding, main app);
// - Coordination of other ViewModels; and
...
final class UserSessionViewModel: ObservableObject {
    ...
    init(context: NSManagedObjectContext = CoreDataStack.shared.viewContext, userLoginRepository: UserLoginRepository = CoreDataUserLoginRepository(), deviceFingerprintRepository: DeviceFingerprintRepository = CoreDataDeviceFingerprintRepository()) {
        ...
        do {
            ...
            if let user = try await userLoginRepository.getCurrentUser() {
                ...
                if let profile = try await userProfileRepository.getProfile(for: user) {
                    ...
                    print("Found profile: displayName='\(profileDisplayName)', dateOfBirth=\(profileDateOfBirth?.description ?? "nil")")
    ...
    func debugCurrentState() async {
        ...
        do {
            ...
            for user in allUsers {
                print("  - User ID: \(user.id), lastLoginAt: \(user.login?.lastLoginAt?.description ?? "nil")")
            }
        } catch {
        ...
        do {
            ...
            for profile in allProfiles {
                print("  - Profile ID: \(profile.id), displayName: '\(profile.displayName)', dateOfBirth: \(profile.dateOfBirth?.description ?? "nil")")
            }
        } catch {
        ...
        if let user = currentUser {
            ...
            do {
                if let profile = try await userProfileRepository.getProfile(for: user) {
                    ...
                    let profileDisplayName = profile.displayName
                    let profileDateOfBirth = profile.dateOfBirth
                    print("Profile found: \(profileDisplayName), DOB: \(profileDateOfBirth?.description ?? "nil")")
                } else {
            ...
            do {
                ...
                if let profile = profiles.first {
                    print("Direct context query found profile: \(profile.displayName), DOB: \(profile.dateOfBirth?.description ?? "nil")")
                } else {
    ...
    func createProfileIfNeeded() async {
        ...
        print("  - tags count: \(userProfileViewModel.tags.count)")
        ...
        do {
            ...
            if let existingProfile = try await userProfileRepository.getProfile(for: user) {
                ...
                try await userProfileRepository.updateProfileWithFormData(
                    existingProfile,
                    ...
                    tags: userProfileViewModel.tags
                )
                print("Profile updated successfully")
            } else {
                print("Creating new profile")
                // Create new profile with form data.
                try await userProfileRepository.createProfileWithFormData(
                    for: user,
                    displayName: userProfileViewModel.displayNameInput,
                    bio: userProfileViewModel.bioInput,
                    dateOfBirth: userProfileViewModel.dateOfBirthInput,
            ...
            let userSettingsRepository = CoreDataUserSettingsRepository()
            do {
